...
- **Altre foto ricevute ma NON usate**: nella cartella `img/` ci sono anche 6 screenshot (`A.S.D. Revolution Sport - Google Chrome ....png`) che mostrano una photosphere di Google Maps con la dicitura "Le immagini possono essere soggette a copyright" attribuita a terzi ("LocalWeb") — non le ho usate sul sito per rischio di copyright su materiale non originale. Se vuoi, sostituiscile con foto scattate direttamente in palestra, oppure eliminale dalla cartella.
...
- **Social**: aggiornare i link Facebook/Instagram (attualmente puntano alle homepage generiche) e il link "Lascia una recensione su Google" (serve il `placeid` reale della palestra).
